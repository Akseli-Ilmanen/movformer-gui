...
def apply_view_settings(plot_item, app_state, preserve_xlim=None):
    ...
    # Set y-limits
    ymin = app_state.get_with_default("ymin")
    ymax = app_state.get_with_default("ymax")
    ...
    if preserve_xlim and len(preserve_xlim) == 2:
        ...
        if window_size > 0 and len(time) > 0:
            t0 = float(time[0])
