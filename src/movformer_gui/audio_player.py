...
class AudioPlayer(QWidget):
    ...
    def __init__(self, napari_viewer, app_state=None, parent=None):
        ...
        self.audio_player = PlayAudio(verbose=0)
    ...
    def _connect_napari_events(self):
        ...
        # Connect to napari's dimension events to sync with video frame changes
        if hasattr(self.viewer.dims, 'events'):
            self.viewer.dims.events.current_step.connect(self._on_napari_frame_changed)
    

    
    def set_audio_loader(self, audio_loader: AudioLoader):
    ...
    def _start_playback(self, start_time=None):
        ...
        if start_time is None:
            try:
                ...
                start_time = 0.0 # in case current_frame = 0
    ...
    # Seeking helpers removed for simplicity; we always align to napari.
