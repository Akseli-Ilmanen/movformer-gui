...
from qtpy.QtWidgets import (
    ...
    QHeaderView, QAbstractItemView, QDialog, QPushButton, QHBoxLayout
)
...
class ShortcutsDialog(QDialog):
    ...
    def _populate_shortcuts_table(self):
        ...
        shortcuts_data = [
            ...
            # Motif Labeling
            ("1", "Motif Label", "Motif 1 - Press key, then click twice on plot to define start/end"),
            ("2", "Motif Label", "Motif 2 - Press key, then click twice on plot to define start/end"),
            ("3", "Motif Label", "Motif 3 - Press key, then click twice on plot to define start/end"),
            ("4", "Motif Label", "Motif 4 - Press key, then click twice on plot to define start/end"),
            ("5", "Motif Label", "Motif 5 - Press key, then click twice on plot to define start/end"),
            ("6", "Motif Label", "Motif 6 - Press key, then click twice on plot to define start/end"),
            ("7", "Motif Label", "Motif 7 - Press key, then click twice on plot to define start/end"),
            ("8", "Motif Label", "Motif 8 - Press key, then click twice on plot to define start/end"),
            ("9", "Motif Label", "Motif 9 - Press key, then click twice on plot to define start/end"),
            ("0", "Motif Label", "Motif 10 - Press key, then click twice on plot to define start/end"),
            ("Q", "Motif Label", "Motif 11 - Press key, then click twice on plot to define start/end"),
            ("W", "Motif Label", "Motif 12 - Press key, then click twice on plot to define start/end"),
            ("R", "Motif Label", "Motif 13 - Press key, then click twice on plot to define start/end"),
            ("T", "Motif Label", "Motif 14 - Press key, then click twice on plot to define start/end"),
            ("", "", ""),  # Empty row for spacing
            
            # Motif Operations
            ("E", "Motif Edit", "Edit selected motif boundaries - Click on motif to select, then press E, then click twice to redefine boundaries"),
            ...
            ("Left Click x2", "Mouse", "After pressing a motif shortcut key (1-9, 0, Q, W, R, T), click twice on the plot to define start and end of new motif region"),
            ("Right Click", "Mouse", "Play motif segment at cursor position - if clicked on an existing motif, plays that segment"),
            ("Right Click x2", "Mouse", "Interrupt current video playback - click right mouse button again while video is playing to stop it"),
        ]
...
class ShortcutsWidget(QWidget):
    ...
    def get_widget(self):
        """Return the widget instance for adding to layouts."""
        return self
    ...
    def _populate_shortcuts_table(self):
        ...
        shortcuts_data = [
            ...
            ("Left Click", "Mouse", "(After motif shortcut button), click 2x to select start/end for new motif"),
            ("Left Click", "Mouse", "Click on motif, then 'D' for delete or 'E + LeftClick + LeftClick' for edit"),
